...
- ✅ **Bayar di Muka Wajib Semua Kanal — DOKU QRIS + Tunai Kasir (fase D1+D2)**: Keputusan user (**revisi**: semua kanal, bukan self-order saja) — bayar di muka Wajib di **self-order, kasir, pelayan**; semua order dibuat `menunggu` (belum ke dapur), baru `diproses` setelah `paid`; **Konfirmasi kasir dihapus** (prepay otomatis ke dapur); meja `terisi` saat `paid`. **Self-order & Pelayan** → QRIS DOKU dinamis (QR di layar HP/tablet pelayan utk discan); **Kasir** → Tunai ATAU QRIS DOKU di muka (bayar segera setelah buat order). Vendor **DOKU** (sandbox demo PKL); pola **B (QRIS di aplikasi + polling)**. Abstraksi `PaymentGateway` (`DokuGateway` + `MockQrisGateway`, pilih via `PAYMENT_DRIVER` default `mock`), kolom baru `payments` (`reference`/`status`/`gateway`/`paid_via`), `paid_at` nullable, endpoint `complete` (Tandai Selesai). **Backend** (D1): `PaymentController::checkout/status/mockPaid/store`, `OrderController::store`→`menunggu`, 22 tes backend ✓. **Frontend** (D2): `QrisPay`, view payment `MenuPage`, overlay pelayan, `PaymentModal` tunai/QRIS, KasirQueuePanel badge Lunas + Tandai Selesai, TransactionHistory + `diproses`, tsc/build/lint/17 tes FE ✓. **D3 (deploy DOKU sandbox online) belum** — lihat checklist Fase D.
- ✅ **Self-order pilih metode bayar + barcode-URL & scan kamera kasir (lanjutan D2)**: Atas keinginan user pelanggan bisa pilih **"Bayar di Kasir"** selain QRIS. **Keputusan user**: (1) kasir membayar order `menunggu` (tab Masuk → modal Bayar, tanpa ubah backend); (2) nama & no. telepon **di-skip**; (3) barcode berisi **URL** `${origin}/order/{orderNumber}` sehingga HP pelanggan scan → **buka halaman tracking** (bukan "Salin Teks"); (4) kasir bisa **manual + barcode gun USB + kamera** (`ScanCameraModal` pakai `html5-qrcode`, ganti Belakang/Depan; kamera butuh HTTPS saat produksi). Implementasi: route publik `/order/:orderNumber` (`OrderTrackingPage`) + komponen `OrderTracking` diekstrak dari MenuPage; endpoint publik baru `GET /order-status/{orderNumber}` (**getOrders butuh auth** → pelanggan tanpa login 401); `KasirScanBox` di tab Masuk; `lib/scan.ts` (`ORD-\d{4}`, tolerir URL/teks) + 5 tes. **Bug fix**: `PaymentController::store` mengembalikan `PaymentResource($order)` (harusnya model `Payment`) → field payment null → struk rusak; fix `$order->refresh()` + `$order->payment`, tambah assertion JSON di tes. Verifikasi: backend **23/23** (72 assertion), FE build/lint/**22/22** tes; E2E curl self-order → tunai → `diproses`+payment benar.
- ✅ **Bug fix crash Daftar Pesanan Pelayan setelah bayar QRIS (lanjutan D2)**: Layar error React (*"Terjadi masalah saat menampilkan halaman"*) muncul setelah pelayan klik **"Saya Sudah Bayar (Demo)"** → `setView('orders')` merender `OrderList`, dan **seed ORD-0003 masih berstatus legacy `menunggu-konfirmasi`** (di luar map `orderStatusBadge`/`orderStatusBar` di `OrderList.tsx`) → `orderStatusBadge[...]` = `undefined` → `<StatusBadge variant={undefined}>` → `colorMap[undefined].bg` → TypeError → ErrorBoundary. Fix: (1) seeder ORD-0003 status → `menunggu`; (2) `OrderList.tsx` fallback `?? 'neutral'` (badge) & `?? ''` (garis); (3) `StatusBadge.tsx` `colorMap[variant] ?? colorMap.neutral`; (4) tes regresi `StatusBadge.test.tsx` (badge + OrderList render status legacy) → **FE 24/24**, backend **23/23**, `migrate:fresh --seed`.
- ✅ **Foto per varian ukuran (Original/Jumbo)**: Keputusan user — foto berbeda **hanya per ukuran** (rasa pedas tetap level 0-5 tanpa foto terpisah); nama varian `Reguler/Besar` → `Original/Jumbo`; foto diunggah Admin; thumbnail berganti per varian di Menu QR + Kasir + Pelayan. **Backend**: migrasi kolom `image_url` di `menu_item_variants` + fillable + `MenuItemVariantResource` expose `imageUrl`; `MenuItemController` validasi `variants.*.imageUrl` (string) & `variants.*.image` (PNG/JPG/WebP max 2 MB) + helper `resolveVariantImageUrl` simpan ke `photo_disk/menu-items/variants`; seeder rename Original/Jumbo. **Frontend**: tipe `MenuItemVariant.imageUrl?` + `MenuVariantInput.image/imageUrl?`; `httpApi` multipart per varian (`variants[i][image]`, JSON `imageUrl`) + `hasVariantImageFiles`/`appendVariantImageFiles`; `mockApi` sertakan variants+imageUrl; Admin `MenuManagement` baris varian upload foto + preview; helper baru `lib/menuPhoto.ts` `displayPhoto(item, selectedVariantName)` dipakai `MenuPanel`, `WaiterOrder`, `MenuPage` (kartu + FeaturedCard) — klik pill varian juga memilih & menambah. Verifikasi: `migrate:fresh --seed` sukses (varian = Original/Jumbo), backend **23/23** (72 assertion), FE `tsc -b`+build ✓, lint ✓ (warning lama saja), **24/24** tes. Uji manual §9c (kasus 9.21–9.26) ditambahkan di `docs/uji manual.md`.
- ✅ **Audit kesiapan online + perbaikan P1 (15 Sep 2026)**: Audit menyeluruh (produksi VPS + item durability) menemukan & menuntaskan 5 masalah kritis. **(P1#1) Void order lunas** — `OrderController::void()` blokir bila `payment()->exists()` (422) + tombol Batalkan disembunyikan di Kitchen & kartu Nota Kasir (`KitchenPage`, `KasirQueuePanel`); 2 tes. **(P1#2) Broadcast tanpa try-catch** — helper `Controller::safeBroadcastOrderChange()` dipakai 7 titik dispatch (Order/Payment); transaksi DB tetap sukses walau Reverb mati (sebelumnya 500 padahal data tersimpan); 2 tes `BroadcastFailureTest`. **(P1#3) Order ganda saat double-click** — idempotensi `POST /orders` via header `X-Idempotency-Key` (migrasi kolom unik `orders.idempotency_key`, guard store + replay + balapan unique) **&** kunci kirim frontend (`sendingRef` + state `isSending`) + tombol "Bayar di Muka" disabled di Kasir, Pelayan, Menu QR; 4 tes `IdempotencyOrderTest`. **(P1#4) Race checkout QRIS** — `checkout()` dalam transaksi + `Order::lockForUpdate()` + unique `payments.order_id` (migrasi dedupe dulu); 4 tes `CheckoutRaceTest`. **(P1#5) Ketersediaan menu tak dicek saat bayar** — `PaymentController::assertItemsAvailable()` dipanggil di `store()` (tunai) & `checkout()` (QRIS) → 422 bila item jadi Habis; 2 tes. Verifikasi: backend **61/61**, frontend tsc ✓ + vitest **26/26**. Commit: `38841c1`, `e7e5a03`, `ae2b952`, `c3ff705`, `fd9126a`, `3bd110d`, `849406a`. Catatan: 2 migrasi untuk MySQL lokal — `2026_09_15_100000_add_idempotency_key...` & `..._100001_add_unique_order_id...` (jalankan `php artisan migrate`).
- **Koreksi catatan lama (analisis performa di bawah)**: butir **#4 "Tidak ada disabled state saat submit order → duplikasi order"** kini sudah **dituntaskan berlapis** — state `isSubmitting` (sejak sesi optimasi Pelayan) + kunci ref `sendingRef` + idempotensi backend `X-Idempotency-Key` (P1#3).
...
## Fase D - Bayar di Muka Wajib — Semua Kanal (DOKU QRIS + Tunai Kasir)

> **Status: D1 ✅ + D2 ✅ — D3 (deploy DOKU sandbox) belum.** Keputusan user yang sudah disepakati
...
- [x] Migrasi: tambah kolom `reference` (unique), `status` (`pending|paid|failed|expired|cancelled`), `gateway`, `paid_via` pada `payments` (+ `paid_at` nullable di migrasi dasar).
- [x] `app/Services/Payment/PaymentGateway` (interface) + `DokuGateway` + `MockQrisGateway`.
- [x] `config/dinflow.php` (`payment_driver` default `mock`, blok `doku`) + binding di `AppServiceProvider` (singleton).
- [x] `PaymentController`: `POST /orders/{id}/checkout` (QRIS via gateway, simpan `payments` status pending) + `GET /payments/{reference}/status` (polling + auto-confirm saat `paid`) + `POST /payments/{reference}/mock-paid` (hanya Mock).
- [x] `OrderController::store`: **semua kanal** status awal `menunggu` (belum ke dapur); tanpa Konfirmasi; baru `diproses` + broadcast + meja `terisi` setelah pembayaran `paid`.
- [x] `PaymentController::store` (tunai kasir di muka): validasi order `menunggu`, set `paid`, lanjutkan order ke dapur + meja `terisi`.
- [x] Guard kasir: order `menunggu` (belum bayar) tidak masuk dapur; order prepay tak bisa dibayar ganda (409); `paid` tampil "Lunas" (tombol Bayar nonaktif).
- [x] `OrderController::complete` (Tandai Selesai) → `selesai` + meja `perlu-dibersihkan` (karena `selesai` tak lagi dicapai lewat pembayaran).
- [x] `.env.example` + `OrderResource`/`PaymentResource` expose status pembayaran (`reference`/`status`/`gateway`/`paidVia`).
- [x] Feature tests backend (**22 tes, 63 assertions** semua lulus) — `PrepayTest` baru + `OrderPaymentTest` disesuaikan.

### D2. Frontend — alur bayar di muka (Prioritas: P1)
- [x] `types` + `api`/`httpApi`/`mockApi`: `checkoutOrder`, `getPaymentStatus`, `markMockPaid`, `completeOrder`; `OrderStatus` kini `menunggu|diproses|selesai|dibatalkan`.
- [x] `QrisPay` (`components/QrisPay.tsx`): QR + polling `getPaymentStatus` tiap 3s + tombol "Saya Sudah Bayar (Demo)" bila driver `mock`; otomatis `onPaid` saat `paid`.
- [x] `MenuPage`: view `payment` — setelah order dibuat → layar **pilih metode** (QRIS langsung / Bayar di Kasir) → `QrisPay` → paid → tracking (auto).
- [x] **Bayar di Kasir (self-order)**: barcode berisi URL `${origin}/order/ORD-XXXX` + nomor besar; kasir bayar lewat tab Masuk / kotak scan; otomatis ke tracking saat `diproses`.
- [x] **Route & halaman tracking publik**: `/order/:orderNumber` (`OrderTrackingPage`, tanpa login, real-time) + komponen `OrderTracking` (dipakai juga MenuPage).
- [x] **Kotak scan kasir** (tab Masuk): input (barcode gun USB / ketik `ORD-XXXX`) + tombol **Scan Kamera** (`html5-qrcode`, ganti Belakang/Depan); ekstraksi via `lib/scan.ts`.
- [x] Backend pendukung: endpoint publik `GET /order-status/{orderNumber}` + **fix bug** `PaymentResource($order->payment)` di `store` (field payment sebelumnya null).
- [x] `WaiterOrder`/`PelayanPage`: setelah kirim order → overlay `QrisPay` (QRIS DOKU di layar pelayan) → polling → paid → daftar pesanan.
- [x] `PaymentModal` (kasir): bayar di muka — Tunai (seperti sekarang) ATAU QRIS DOKU (checkout → QR → polling → paid).
- [x] Hapus tombol "Konfirmasi" dari `KasirQueuePanel`/flow kasir — semua otomatis ke dapur saat `paid`; Masuk = `menunggu` + "Bayar di Muka".
- [x] `KasirQueuePanel`: Nota (`diproses`) badge "Lunas" + "Tandai Selesai"; `TransactionHistory` tampilkan juga `diproses`.
- [x] Ganti teks "bayar di kasir" → "pembayaran di muka"; tombol kirim → "Bayar di Muka".
